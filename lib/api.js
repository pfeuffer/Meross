...
axiosInstanace.defaults.timeout = 10000;
...
module.exports = class API {
    constructor(host, key, userId, verbose = false) {
        ...
        axiosInstanace.interceptors.request.use(request => {
            if (verbose) {
                logRequest(request)
            }
            return request
        ...
        axiosInstanace.interceptors.response.use(response => {
            if (verbose) {
    ...
    async switchChannel(channel, onoff) {
